...
const positionService = {
  ...
  async add({ name, description }) {
    try {
      const res = await apiConfig.post(endpointConst.POSITION.ADD, { name, description });
      return res.data;
    } catch (error) {
      return error.response?.data || null;
    }
  ...
  async updateById({ id, name, description }) {
    try {
      const res = await apiConfig.put(endpointConst.POSITION.UPDATE(id), { name, description });
      return res.data;
    } catch (error) {
      return error.response?.data || null;
    }
