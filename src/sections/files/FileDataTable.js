...
const columns = [
  ...
  },
  {
    ...
    width: 130,
    valueGetter: (params) =>
      params.row.createdAt && format(new Date(params.row.createdAt), "dd/MM/yyyy"),
  },
  {
    field: "actions",
    headerName: "Actions",
    ...
    type: "actions",
    renderCell: (params) => (
      <Actions fileId={params.row.id} downloadUrl={params.row.url} fileName={params.row.name} />
    ),
  },
];
...
const Actions = ({ fileId, downloadUrl, fileName }) => {
  ...
  return (
    <>
      ...
      >
        <DialogTitle id="alert-dialog-title">{"Are you sure delete this file?"}</DialogTitle>
        <DialogContent>
